 ...
 * groups `zh`/`en`/`es` across sibling files as well as within one module. A
 * `zh` dictionary whose `en` or `es` counterpart cannot be found anywhere is
 * an error, not a skip.
 */
...
function dictionariesIn(file: string): Dictionary[] {
  ...
  // `\b(zh|en|es)\b` misses `zhSettings`/`accessEs`, because `\b` does not
  // hold between a locale tag and an uppercase letter.
  ...
  // — e.g. a local dictionary variable rather than an inline literal —
  // resolves through here so the gate still verifies its symmetry.
  ...
  // ['es', {...}]]` group handed to a registration loop keys off the enclosing
  // array; separate `register(NS, 'zh', {...})` / `register(NS, 'en', {...})`
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isCallExpression(node)) {
      ...
      if (name === 'register' && node.arguments.length >= 3) {
        ...
        if (!isLocaleTag(tag.text)) return
    ...
    if (ts.isArrayLiteralExpression(node) && node.elements.length >= 2) {
      ...
      for (const element of node.elements) {
        ...
        if (literal === undefined || !ts.isObjectLiteralExpression(literal)) continue
        if (!isLocaleTag(tag.text)) continue
...
/** Locale tags this gate tracks: the full set shipped by the browser client. */
type ShippedLocale = 'zh' | 'en' | 'es'

/** The tracked locale tags, in source-of-truth order (zh first). */
const SHIPPED_LOCALES: readonly ShippedLocale[] = ['zh', 'en', 'es']

/** Whether a string literal names a shipped locale tag. */
function isLocaleTag(text: string): text is ShippedLocale {
  return (SHIPPED_LOCALES as readonly string[]).includes(text)
}

/**
 * The locale a dictionary name declares, and the namespace-ish remainder that
 * identifies which group it belongs to. `zh`/`en`/`es`,
 * `zhSettings`/`enSettings`/`esSettings`, and `settingsZh`/`settingsEn`/
 * `settingsEs` are the shapes this repo uses. A name-prefix shape requires an
 * uppercase ASCII letter at the third position (`[A-Z]`), matching the
 * admission of the cheap pre-filter, so `zh2Foo`/`zh_probe` cannot be treated
 * as dictionaries in one place and skipped in another.
...
function localeOf(name: string): { locale: ShippedLocale; pair: string } | undefined {
  for (const locale of SHIPPED_LOCALES) {
    ...
    // Synthetic names for inline shapes carry their own group key after the
...
describe('shipped locale dictionaries', () => {
  it('declares the same keys in zh, en, and es, so the single fallback locale always resolves', () => {
    const files = sourceFiles()
    // Guard the discovery itself: an empty or narrowed sweep would pass every
    // assertion below while checking nothing.
    expect(files.length).toBeGreaterThan(500)

    // Group within a file first; a dictionary whose counterpart is not in the
    // same module then groups with a sibling in the same directory. Both
    // shapes ship here: `locales/settings.ts` exports zh+en+es together,
    // while `locales/zh.ts` + `locales/en.ts` + `locales/es.ts` split the
    // common group across files.
    ...
    const place = (key: string, locale: ShippedLocale, dict: Dictionary): void => {
      ...
      if (slot.has(locale)) {
        throw new Error(`two ${locale} dictionaries claim group ${key}: ${slot.get(locale)?.file} and ${dict.file}`)
      }
    ...
    for (const [key, slot] of [...groups].sort()) {
      ...
      if (zh === undefined || en === undefined || es === undefined) {
        const present = zh ?? en ?? es
        const missing = SHIPPED_LOCALES.filter(locale => !slot.has(locale)).join('/')
        problems.push(`${present?.file} declares ${present?.name} with no ${missing} counterpart for group ${key}`)
        continue
      }
      comparedGroups++
      for (const other of [en, es]) {
