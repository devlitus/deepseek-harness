...
读取浏览器修好了那些浏览器声明了本应用所提供语言的读者，但残余情形依然是错的：`zh`、`en`、`es` 都不请求的浏览器（`fr`、`de`）仍会回落到 `zh`。这些读者恰恰最不可能阅读中文。
...
- **完整的 BCP 47 协商（`Intl.LocaleMatcher` 式查找、地区与文字权重）**：已提供的 locale 仅在语言层面互异（无地区变体），此时主子标签匹配就是正确答案的全部；协商层只会带来无行为支撑、也无从测试的表面积。
