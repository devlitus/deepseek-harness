...
**Component copy rides the standard `t` seat; deep children take `t` as a plain prop** typed `XxxProps['t']`. The dictionary canon is unchanged: `zh satisfies Record<string, string>` is the key source and the `en`/`es` dictionaries lock all-locale balance (`satisfies Record<XxxKey, string>` or the file's equivalent).
